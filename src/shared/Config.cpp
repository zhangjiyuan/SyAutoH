...
//if the first is spaces, then delete it
...
void remove_all_spaces(string & str)
{
	...
	while(off != string::npos)               // npos find the end of the string 
	{
		str.erase(off, 1);
		off = str.find(" ");
	}
...
{
	register size_t len = strlen(str);                      //register 希望编译器尽量将变量存储在CPU寄存器中，而不是通过内存寻址提高效率
...
bool ConfigFile::SetSource(const char* file, bool ignorecase)
{
	...
	if(file != 0)
	{
...
#ifdef WIN32
		FILE* f = fopen(file, "rb");                 //以读写二进制的形式打开文件，返回指向文件流的文件指针
#else
		FILE* f = fopen(file, "r");
#endif
		char* buf;
		...
		fseek(f, 0, SEEK_END);                //文件指针转到文件尾
		length = ftell(f);
		buf = new char[length + 1];
		fseek(f, 0, SEEK_SET);                //文件指针返回文件首     
		...
		bool in_multiline_comment = false;      //注释
		bool in_multiline_quote = false;        //引用
		bool in_block = false;                  //块
...
std::string ConfigFile::GetStringDefault(const char* block, const char* name, const char* def)                    //获得string的默认值
{
	string ret;
