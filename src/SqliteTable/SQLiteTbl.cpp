...
void SQLiteString::operator +=(const char* pStr)
{
    ...
    if (pData && pStr) 
	{
        ...
        strcpy_s(pBuf, nStrLen, pData);
        strcat_s(pBuf, nStrLen, pStr);
        SetString(pBuf);
        delete [] pBuf;
    }
}
...
{	
    ...
    char* pBuf = new char[STR_LEN];
    va_list ap;
    
    va_start(ap, fmt);
    vsprintf_s(pBuf, STR_LEN, fmt, ap);
    SetString(pBuf);
    va_end(ap);
    delete [] pBuf;
} 
						...
						bool bIndex)
{
    ...
    if (m_pItems && item < size) 
	{
        if (strlen(pName) < 100)
		{
            strcpy_s(m_pItems[item].name, strlen(pName)+1, pName);
            m_pItems[item].type = type;
            m_pItems[item].pData = pData;
            m_pItems[item].item_col = -1; //init it to -1
			m_pItems[item].bIndex = bIndex;
        }
    }
}

bool SQLiteTbl::Open(sqlite3* pDB)
{
	if (SQLiteTbl::OpenDB(pDB)) 
	{
		//if (Query()) 
		if (LoadTable()) 
			return true;
...
bool SQLiteTbl::OpenDB(sqlite3* pDB)
{
	if (NULL != pDB)
	{
		m_is_open = true;
...
//CREATE UNIQUE INDEX [IX_Login] ON [LoginInfo] ([Login]);
bool SQLiteTbl::InsertIndex()
{
	...
	if (m_is_open && m_pDb && m_pItems && item_count) 
	{
		...
		{
			if (true == m_pItems[x].bIndex)
			{
				sql.Format("CREATE INDEX [IX_%s] ON [%s] ([%s]);", 
					m_pItems[x].name, TableName(), m_pItems[x].name);
				ExeSQL(sql);
			}
		}
    ...
    if (m_is_open && m_pDb && m_pItems && item_count) 
	{
        ...
        for (int x=0; x < item_count; x++) {
            ...
            case MYDB_TYPE_INT:
                ...
                break;
			case MYDB_TYPE_LONG:
				ss.Format("%c[%s] integer", sep, m_pItems[x].name);
				break;
			case MYDB_TYPE_ID:
				ss.Format("%c[%s] integer PRIMARY KEY AUTOINCREMENT NOT NULL", sep, m_pItems[x].name);
				break;
...
SQLiteString SQLiteTbl::GetSQL(bool rowid_only, bool use_filter, bool use_sort)
{
    SQLiteString sql;
    ...
    if (use_sort && !m_sort.IsEmpty()) 
	{
		...
		if (m_row_limit > 0)
		{
			SQLiteString strLimit;
			strLimit.Format(" LIMIT %d", m_row_limit);
			sql += strLimit;
		}
		...
		{
			SQLiteString strLimit;
			strLimit.Format(" OFFSET %d", m_row_skip);
			sql += strLimit;
		}
    }
...
bool SQLiteTbl::Query()
{
    ...
    if (m_is_open && m_pDb) {
		...
		if ((m_row_count > m_row_limit) && (m_row_limit > 0) )
		{
			m_row_count = m_row_limit;
		}
        m_row_pos = 1;
        ...
        for (int c =0; c < item_count; c++) {
            ...
            case MYDB_TYPE_INT:
                ...
                *pInt = 0;
                break;
			case MYDB_TYPE_LONG:
				pLLong = (long long*)m_pItems[c].pData;
				*pLLong = 0;
				break;
            case MYDB_TYPE_DOUBLE:
            ...
            if (m_pItems[i].pData) {
                ...
                case MYDB_TYPE_INT:
                    *((int*)m_pItems[i].pData) = sqlite3_column_int(pStmt, m_pItems[i].item_col); 
                    break;
				case MYDB_TYPE_LONG:
					*((long long*)m_pItems[i].pData) = sqlite3_column_int64(pStmt, m_pItems[i].item_col); 
					break;
				case MYDB_TYPE_ID:
					*((int*)m_pItems[i].pData) = sqlite3_column_int(pStmt, m_pItems[i].item_col); 
					break;
    ...
    if (m_is_open && m_pDb && m_pItems && (m_rowid_edit > 0 || m_add)) {
		...
		{  
            ...
            for (int c =0; c < item_count; c++) 
			{
				...
				case MYDB_TYPE_LONG:                        
					pLLong = (long long*)m_pItems[c].pData;
					sqlite3_bind_int64(pStmt, bind_idx++, *pLLong);                            
					break;
				case MYDB_TYPE_ID:   
					if (m_rowid_edit > 0)
					{
						sqlite3_bind_int(pStmt, bind_idx++, m_rowid_edit);       
					}
					else
					{
						sqlite3_bind_null(pStmt, bind_idx++);        
					}
...
void SQLiteTbl::SetDBPtr(sqlite3 * pDb)
{
	m_pDb = pDb;
}

bool SQLiteTbl::LoadTable(void)
{
	if (!LoadColumnNumbers())
	{
		Create();
...
void SQLiteTbl::BeginTrans(void)
{
	ExeSQL("begin transaction;");
}

bool SQLiteTbl::EndTrans(void)
{
	return ExeSQL("commit transaction;");
}